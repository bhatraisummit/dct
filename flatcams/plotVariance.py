...
def variance_quarters(data_path):
    ...
    for i in range(range_len):
        Y0.append(np.std(X0[:, i], dtype=np.float64))
        Y1.append(np.std(X1[:, i], dtype=np.float64))
        Y2.append(np.std(X2[:, i], dtype=np.float64))
        Y3.append(np.std(X3[:, i], dtype=np.float64))
    print(max(Y0))
    print(max(Y1))
    print(max(Y2))
    print(max(Y3))
    ...
    print(min(Y2))
    print(min(Y3))
    np.save('variance/Y0', Y0)
    np.save('variance/Y1', Y1)
    np.save('variance/Y2', Y2)
    np.save('variance/Y3', Y3)
...
def add_noise_std (arr, loc, std, SEED):
    ...
    return arr.reshape(reshape_size), zero_arr.reshape(reshape_size)


def add_noise(path, k):
    calib = loadmat('flatcam_calibdata.mat')  # load calibration data
    flatcam.clean_calib(calib)
    Y0_path = 'variance/Y0_loc.npy'
    Y1_path = 'variance/Y1_loc.npy'
    Y2_path = 'variance/Y2_loc.npy'
    Y3_path = 'variance/Y3_loc.npy'
    ...
    std = 75
    x0, x0n = add_noise_std (x0, Y0_loc, std, 5)
    x1, x1n = add_noise_std (x1, Y1_loc, std, 15)
    x2, x2n = add_noise_std (x2, Y2_loc, std, 25)
    x3, x3n = add_noise_std (x3, Y3_loc, std, 35)
    dct2_im = multiresolution.join_quarters(x0, x1, x2, x3)
    idct2_im_n = multiresolution.idct2Dimg(dct2_im)
    ...
    recon_images_n.save(f'{path.split("/")[-1].split(".")[0]}_{std}_{round(psnr,2)}.png')
    ...
    psnr = metrics.peak_signal_noise_ratio(np.asarray(recon_orig_pil).astype(np.uint8),
                                          np.asarray(recon_images_n).astype(np.uint8))
    recon_images_n.save(f'{path.split("/")[-1].split(".")[0]}_{std}_{round(psnr,2)}_zn.png')
...
if __name__ == '__main__':
    ...
    Y0_path = 'variance/Y0.npy'
    Y1_path = 'variance/Y1.npy'
    Y2_path = 'variance/Y2.npy'
    Y3_path = 'variance/Y3.npy'
    # save_hist(Y0_path)
    # save_hist(Y1_path)
    # save_hist(Y2_path)
    # save_hist(Y3_path)
    Y0_loc = find_locations(Y0_path, 1, 0.433308543, 106.19)
    Y1_loc = find_locations(Y1_path, 2, 1.02253524, 3.1116706)
    Y2_loc = find_locations(Y2_path, 2, 1.06196287, 3.84049905)
    Y3_loc = find_locations(Y3_path, 5, 0.58045218, 1.14540819)
    ...
    np.save('variance/Y0_loc', Y0_loc)
    np.save('variance/Y1_loc', Y1_loc)
    np.save('variance/Y2_loc', Y2_loc)
    np.save('variance/Y3_loc', Y3_loc)
    ...
    paths = ['/scratch/s571b087/project/Lensless_Imaging/rice_face/fc_captures/10/115.png',
        ...
        , '/scratch/s571b087/project/Lensless_Imaging/rice_face/fc_captures/15/079.png']
    #for i in range (0,16):
    for path in paths:
        add_noise(path, 1)
       # os.system(f'mkdir {i}')
